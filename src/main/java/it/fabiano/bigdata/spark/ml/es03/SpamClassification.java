package it.fabiano.bigdata.spark.ml.es03;
...
public final class SpamClassification {
	...
	public static void main(String[] args) {
		SparkConf sparkConf = new SparkConf().setAppName("MLibExample").setMaster("local[*]");
		JavaSparkContext sc = new JavaSparkContext(sparkConf);

		// Load 2 types of emails from text files: spam and ham (non-spam).
		// Each line has text from one email.
		JavaRDD<String> spam = sc.textFile("in/spam.txt");
		JavaRDD<String> ham = sc.textFile("in/ham.txt");
		...
		Vector posTestExample =
				tf.transform(Arrays.asList("cheap Viagra for yoy by sending money to ...".split(" ")));
